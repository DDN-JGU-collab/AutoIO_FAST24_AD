...
io500_mpirun="mpirun"
#io500_mpiargs="--bind-to core:overload-allowed --allow-run-as-root singularity_qian.sh /exafs -B /work -B /usr -B /sys/class/infiniband_verbs -B /bin -B /sbin -B /etc/libibverbs.d centos8.sif "
...
function setup(){
  ...
  # overstripe
  lfs setstripe -C 256 $workdir/ior-hard
  # Metadata OverStriping
  #lfs setdirstripe -D -C 20 $workdir/mdtest-hard
  lfs setdirstripe -D -c 4 $workdir/mdtest-hard
  lfs setdirstripe -D -c 4 $workdir/mdtest-easy
